...
if [[ "$A_INPUT" != /* ]]; then A_INPUT="$PROJECT_DIR/$A_INPUT"; fi
if [[ "$B_INPUT" != /* ]]; then B_INPUT="$PROJECT_DIR/$B_INPUT"; fi
A_INPUT="$(realpath "$A_INPUT")"
B_INPUT="$(realpath "$B_INPUT")"
if [[ ! -f "$A_INPUT" ]]; then echo "ERROR: Matrix A not found: $A_INPUT"; exit 1; fi
if [[ ! -f "$B_INPUT" ]]; then echo "ERROR: Matrix B not found: $B_INPUT"; exit 1; fi
for NODE_COUNT in 1 2 3; do
for PROCESS_COUNT in 1 2 4 8 16; do
if (( PROCESS_COUNT < NODE_COUNT )); then continue; fi
sbatch --nodes="$NODE_COUNT" --ntasks="$PROCESS_COUNT" --cpus-per-task=1 --export=ALL,A_INPUT="$A_INPUT",B_INPUT="$B_INPUT" "$PROJECT_DIR/run_rce.sh" --grid-job
done
done
exit 0
fi
...
if [[ "$A_FILE" != /* ]]; then A_FILE="$PROJECT_DIR/$A_FILE"; fi
if [[ "$B_FILE" != /* ]]; then B_FILE="$PROJECT_DIR/$B_FILE"; fi
A_FILE="$(realpath "$A_FILE")"
B_FILE="$(realpath "$B_FILE")"
TASKS="${SLURM_NTASKS:-1}"
JOB_ID="${SLURM_JOB_ID:-manual}"
NODE_COUNT="${SLURM_JOB_NUM_NODES:-1}"
TASKS_PER_NODE=$(( (TASKS + NODE_COUNT - 1) / NODE_COUNT ))
...
THROUGHPUT=$(awk "BEGIN { if ($TOTAL_SECONDS > 0) printf \"%.6f\", $M / $TOTAL_SECONDS; else print \"0\" }")
...
echo "$N,$M,$P,$TASKS,$NODE_COUNT,$M,$A_BYTES,$B_BYTES,$SPLIT_SECONDS,$MAPPER_SECONDS,$SHUFFLE_SECONDS,$REDUCER_SECONDS,$TOTAL_SECONDS,$OUTPUT_BYTES,$THROUGHPUT,$A_FILE,$JOB_ID" >> "$BENCHMARK_FILE"
echo ""
echo "=========================================="
echo "JOB COMPLETED"
echo "=========================================="
